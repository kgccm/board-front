...
const usePagination = <T>(countPerPage: number) => {
  ...
  useEffect(() => {
    const totalPage = Math.ceil(totalList.length / countPerPage); // 전체 객체 리스트를 카운트로 나눈 뒤 올림처리
    const totalPageList: number[] = [];
    for (let page = 1; page <= totalPage; page++) totalPageList.push(page);
    setTotalPageList(totalPageList);

    const totalSection = Math.ceil(totalList.length / (countPerPage * 10)); // 전체 섹션개수를 카운트로 나눈 뒤 10개씩 보여주기 위한 작업(올림처리)
