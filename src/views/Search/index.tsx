...
import { getRelationListRequest, getSearchBoardListRequest, getSearchRecipeListRequest } from 'apis';
...
export default function Search() {
  ...
  useEffect(() => {
    if (!searchWord) return;
    if (selectedItem === 'board') getSearchBoardListRequest(searchWord, preSearchWord).then(getSearchBoardListResponse);
    else getSearchRecipeListRequest(searchWord, preSearchWord).then(getSearchRecipeListResponse);
        ...
        <div className='search-contents-box'>
          {count === 0 ?
            ...
            <div className='search-contents'>
              {selectedItem === 'board' && viewList.map(boardListItem => <BoardItem boardListItem={boardListItem} />)}
              {selectedItem === 'recipe' && viewList.map(recipeListItem => <RecipeItem recipeListItem={recipeListItem} />)}
            </div>
          }
